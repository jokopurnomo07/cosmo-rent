...
class ReservationController extends Controller
{
    public function index($status)
    {
        ...
        $reservation = Reservation::with(['user', 'vehicle', 'rental_package', 'services'])
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($reservation) {
            ...
            if ($request->status == 'rejected' || $request->status == "canceled") {
                ...
                $emailLabel = $request->status == "rejected" ? "Penolakan" : "Pembatalan";
                Mail::to($reservation->user_id != null ? $reservation->user->email : $reservation->email_guest)
